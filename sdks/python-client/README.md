...
A Aera App Service-API Client, using for build a webapp by request Service-API

## Usage

First, install `aera-client` python sdk package:

```
pip install aera-client
```
...
```python
from aera_client import CompletionClient
...
```python
import json
from aera_client import ChatClient
...
```python
from aera_client import AeraClient

api_key = "your_api_key"

# Initialize Client
aera_client = AeraClient(api_key)
...
with open(file_path, "rb") as file:
    ...
    response = aera_client.file_upload("user_id", files)
