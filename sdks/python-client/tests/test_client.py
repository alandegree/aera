...
from aera_client.client import (
    ChatClient,
    CompletionClient,
    AeraClient,
    KnowledgeBaseClient,
...
API_BASE_URL = os.environ.get("API_BASE_URL", "https://api.aera.ac/v1")
...
class TestAeraClient(unittest.TestCase):
    def setUp(self):
        self.aera_client = AeraClient(API_KEY)

    def test_message_feedback(self):
        response = self.aera_client.message_feedback(
            "your_message_id", "like", "test_user"
        )
        self.assertIn("success", response.text)

    def test_get_application_parameters(self):
        response = self.aera_client.get_application_parameters("test_user")
    ...
    def test_file_upload(self):
        ...
        with open(file_path, "rb") as file:
            ...
            response = self.aera_client.file_upload("test_user", files)
