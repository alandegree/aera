...
class AeraClient:
    def __init__(self, api_key, base_url: str = "https://api.aera.ac/v1"):
...
class CompletionClient(AeraClient):
...
class WorkflowClient(AeraClient):
    def run(
...
class KnowledgeBaseClient(AeraClient):
    def __init__(
        self,
        api_key,
        base_url: str = "https://api.aera.ac/v1",
        ...
        Args:
            api_key (str): API key of Aera.
            base_url (str, optional): Base URL of Aera API. Defaults to 'https://api.aera.ac/v1'.
