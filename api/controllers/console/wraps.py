...
def only_edition_self_hosted(view):
    @wraps(view)
    def decorated(*args, **kwargs):
        if aera_config.EDITION != "SELF_HOSTED":
            abort(404)
            ...
            if features.billing.enabled:
                if resource == "add_segment":
                    if features.billing.subscription.plan == "sandbox":
                        abort(
                            403,
                            "To unlock this feature and elevate your Aera experience, please upgrade to a paid plan.",
                        )
                else:
                    return view(*args, **kwargs)
...
def setup_required(view):
    ...
    def decorated(*args, **kwargs):
        # check setup
        if (
            aera_config.EDITION == "SELF_HOSTED"
            and os.environ.get("INIT_PASSWORD")
            and not db.session.query(AeraSetup).first()
        ):
            raise NotInitValidateError()
        elif aera_config.EDITION == "SELF_HOSTED" and not db.session.query(AeraSetup).first():
            raise NotSetupError()
