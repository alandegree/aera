...
class AeraSetup(Base):
    __tablename__ = "aera_setups"
    __table_args__ = (db.PrimaryKeyConstraint("version", name="aera_setup_pkey"),)
...
class App(Base):
    ...
    def api_base_url(self):
        return (aera_config.SERVICE_API_URL or request.host_url.rstrip("/")) + "/v1"
...
class Conversation(db.Model):  # type: ignore[name-defined]
    ...
    def inputs(self):
        ...
        for key, value in inputs.items():
            ...
            if isinstance(value, dict) and value.get("aera_model_identity") == FILE_MODEL_IDENTITY:
                ...
                isinstance(item, dict) and item.get("aera_model_identity") == FILE_MODEL_IDENTITY for item in value
            ):
                inputs[key] = []
                for item in value:
...
class Site(Base):
    ...
    def app_base_url(self):
        return aera_config.APP_WEB_URL or request.url_root.rstrip("/")
