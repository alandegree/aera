...
class SystemFeatureModel(BaseModel):
    ...
    max_plugin_package_size: int = aera_config.PLUGIN_MAX_PACKAGE_SIZE
...
class FeatureService:
    ...
    def get_features(cls, tenant_id: str) -> FeatureModel:
        ...
        if aera_config.BILLING_ENABLED and tenant_id:
    ...
    def get_system_features(cls) -> SystemFeatureModel:
        ...
        if aera_config.ENTERPRISE_ENABLED:
            system_features.enable_web_sso_switch_component = True

            cls._fulfill_params_from_enterprise(system_features)

        if aera_config.MARKETPLACE_ENABLED:
    ...
    def _fulfill_system_params_from_env(cls, system_features: SystemFeatureModel):
        system_features.enable_email_code_login = aera_config.ENABLE_EMAIL_CODE_LOGIN
        system_features.enable_email_password_login = aera_config.ENABLE_EMAIL_PASSWORD_LOGIN
        system_features.enable_social_oauth_login = aera_config.ENABLE_SOCIAL_OAUTH_LOGIN
        system_features.is_allow_register = aera_config.ALLOW_REGISTER
        system_features.is_allow_create_workspace = aera_config.ALLOW_CREATE_WORKSPACE
        system_features.is_email_setup = aera_config.MAIL_TYPE is not None and aera_config.MAIL_TYPE != ""

    @classmethod
    def _fulfill_params_from_env(cls, features: FeatureModel):
        features.can_replace_logo = aera_config.CAN_REPLACE_LOGO
        features.model_load_balancing_enabled = aera_config.MODEL_LB_ENABLED
        features.dataset_operator_enabled = aera_config.DATASET_OPERATOR_ENABLED
