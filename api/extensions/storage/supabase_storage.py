...
class SupabaseStorage(BaseStorage):
    ...
    def __init__(self):
        super().__init__()
        if aera_config.SUPABASE_URL is None:
            raise ValueError("SUPABASE_URL is not set")
        if aera_config.SUPABASE_API_KEY is None:
            raise ValueError("SUPABASE_API_KEY is not set")
        if aera_config.SUPABASE_BUCKET_NAME is None:
            raise ValueError("SUPABASE_BUCKET_NAME is not set")

        self.bucket_name = aera_config.SUPABASE_BUCKET_NAME
        self.client = Client(supabase_url=aera_config.SUPABASE_URL, supabase_key=aera_config.SUPABASE_API_KEY)
        self.create_bucket(id=aera_config.SUPABASE_BUCKET_NAME, bucket_name=aera_config.SUPABASE_BUCKET_NAME)
