...
def init_app(app: AeraApp):
    ...
    def after_request(response):
        ...
        response.headers.add("X-Version", aera_config.CURRENT_VERSION)
        response.headers.add("X-Env", aera_config.DEPLOY_ENV)
        return response

    @app.route("/health")
    def health():
        return Response(
            json.dumps({"pid": os.getpid(), "status": "ok", "version": aera_config.CURRENT_VERSION}),
            status=200,
