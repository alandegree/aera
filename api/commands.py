...
def migrate_knowledge_vector_database():
    ...
    skipped_count = 0
    total_count = 0
    vector_type = aera_config.VECTOR_STORE
...
def add_qdrant_doc_id_index(field: str):
    ...
    try:
        ...
        for binding in bindings:
            if aera_config.QDRANT_URL is None:
                raise ValueError("Qdrant URL is required.")
            qdrant_config = QdrantConfig(
                endpoint=aera_config.QDRANT_URL,
                api_key=aera_config.QDRANT_API_KEY,
                root_path=current_app.root_path,
                timeout=aera_config.QDRANT_CLIENT_TIMEOUT,
                grpc_port=aera_config.QDRANT_GRPC_PORT,
                prefer_grpc=aera_config.QDRANT_GRPC_ENABLED,
            )
            try:
