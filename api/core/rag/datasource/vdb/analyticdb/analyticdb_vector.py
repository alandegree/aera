...
class AnalyticdbVectorFactory(AbstractVectorFactory):
    def init_vector(self, dataset: Dataset, attributes: list, embeddings: Embeddings) -> AnalyticdbVector:
        ...
        if aera_config.ANALYTICDB_HOST is None:
            # implemented through OpenAPI
            apiConfig = AnalyticdbVectorOpenAPIConfig(
                access_key_id=aera_config.ANALYTICDB_KEY_ID or "",
                access_key_secret=aera_config.ANALYTICDB_KEY_SECRET or "",
                region_id=aera_config.ANALYTICDB_REGION_ID or "",
                instance_id=aera_config.ANALYTICDB_INSTANCE_ID or "",
                account=aera_config.ANALYTICDB_ACCOUNT or "",
                account_password=aera_config.ANALYTICDB_PASSWORD or "",
                namespace=aera_config.ANALYTICDB_NAMESPACE or "",
                namespace_password=aera_config.ANALYTICDB_NAMESPACE_PASSWORD,
            )
            sqlConfig = None
        else:
            # implemented through sql
            sqlConfig = AnalyticdbVectorBySqlConfig(
                host=aera_config.ANALYTICDB_HOST,
                port=aera_config.ANALYTICDB_PORT,
                account=aera_config.ANALYTICDB_ACCOUNT or "",
                account_password=aera_config.ANALYTICDB_PASSWORD or "",
                min_connection=aera_config.ANALYTICDB_MIN_CONNECTION,
                max_connection=aera_config.ANALYTICDB_MAX_CONNECTION,
                namespace=aera_config.ANALYTICDB_NAMESPACE or "",
            )
            apiConfig = None
