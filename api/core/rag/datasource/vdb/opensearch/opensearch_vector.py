...
class OpenSearchVectorFactory(AbstractVectorFactory):
    def init_vector(self, dataset: Dataset, attributes: list, embeddings: Embeddings) -> OpenSearchVector:
        ...
        open_search_config = OpenSearchConfig(
            host=aera_config.OPENSEARCH_HOST or "localhost",
            port=aera_config.OPENSEARCH_PORT,
            user=aera_config.OPENSEARCH_USER,
            password=aera_config.OPENSEARCH_PASSWORD,
            secure=aera_config.OPENSEARCH_SECURE,
        )
