...
class RelytVectorFactory(AbstractVectorFactory):
    def init_vector(self, dataset: Dataset, attributes: list, embeddings: Embeddings) -> RelytVector:
        ...
        return RelytVector(
            ...
            config=RelytConfig(
                host=aera_config.RELYT_HOST or "localhost",
                port=aera_config.RELYT_PORT,
                user=aera_config.RELYT_USER or "",
                password=aera_config.RELYT_PASSWORD or "",
                database=aera_config.RELYT_DATABASE or "default",
            ),
            group_id=dataset.id,
        )
