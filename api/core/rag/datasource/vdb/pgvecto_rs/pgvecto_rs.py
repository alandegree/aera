    ...
    def init_vector(self, dataset: Dataset, attributes: list, embeddings: Embeddings) -> PGVectoRS:
        ...
        return PGVectoRS(
            ...
            config=PgvectoRSConfig(
                host=aera_config.PGVECTO_RS_HOST or "localhost",
                port=aera_config.PGVECTO_RS_PORT or 5432,
                user=aera_config.PGVECTO_RS_USER or "postgres",
                password=aera_config.PGVECTO_RS_PASSWORD or "",
                database=aera_config.PGVECTO_RS_DATABASE or "postgres",
            ),
            dim=dim,
        )
