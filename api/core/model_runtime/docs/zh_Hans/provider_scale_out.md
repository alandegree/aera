...
- `fetch-from-remote` 从远程获取
  ...
  如OpenAI，我们可以基于gpt-turbo-3.5来Fine Tune多个模型，而他们都位于同一个**api_key**下，当配置为 `fetch-from-remote` 时，开发者只需要配置统一的**api_key**即可让AeraRuntime获取到开发者所有的微调模型并接入Aera。
...
供应商需要继承 `__base.model_provider.ModelProvider` 基类，实现 `validate_provider_credentials` 供应商统一凭据校验方法即可，可参考 [AnthropicProvider](https://github.com/langgenius/aera-runtime/blob/main/lib/model_providers/anthropic/anthropic.py)。
