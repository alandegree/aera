...
SSRF_DEFAULT_MAX_RETRIES = aera_config.SSRF_DEFAULT_MAX_RETRIES
...
def make_request(method, url, max_retries=SSRF_DEFAULT_MAX_RETRIES, **kwargs):
    ...
    if "timeout" not in kwargs:
        kwargs["timeout"] = httpx.Timeout(
            timeout=aera_config.SSRF_DEFAULT_TIME_OUT,
            connect=aera_config.SSRF_DEFAULT_CONNECT_TIME_OUT,
            read=aera_config.SSRF_DEFAULT_READ_TIME_OUT,
            write=aera_config.SSRF_DEFAULT_WRITE_TIME_OUT,
        )

    retries = 0
    while retries <= max_retries:
        try:
            if aera_config.SSRF_PROXY_ALL_URL:
                with httpx.Client(proxy=aera_config.SSRF_PROXY_ALL_URL) as client:
                    response = client.request(method=method, url=url, **kwargs)
            elif aera_config.SSRF_PROXY_HTTP_URL and aera_config.SSRF_PROXY_HTTPS_URL:
                proxy_mounts = {
                    "http://": httpx.HTTPTransport(proxy=aera_config.SSRF_PROXY_HTTP_URL),
                    "https://": httpx.HTTPTransport(proxy=aera_config.SSRF_PROXY_HTTPS_URL),
                }
