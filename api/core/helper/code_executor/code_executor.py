...
class CodeExecutor:
    ...
    def execute_code(cls, language: CodeLanguage, preload: str, code: str) -> str:
        ...
        :return:
        """
        url = URL(str(aera_config.CODE_EXECUTION_ENDPOINT)) / "v1" / "sandbox" / "run"

        headers = {"X-Api-Key": aera_config.CODE_EXECUTION_API_KEY}
        ...
        try:
            response = post(
                ...
                headers=headers,
                timeout=Timeout(
                    connect=aera_config.CODE_EXECUTION_CONNECT_TIMEOUT,
                    read=aera_config.CODE_EXECUTION_READ_TIMEOUT,
                    write=aera_config.CODE_EXECUTION_WRITE_TIMEOUT,
                    pool=None,
                ),
            )
