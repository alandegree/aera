...
import MoaeraRetrievalModal from './moaera-retrieval-modal'
...
const HitTestingPage: FC<Props> = ({ datasetId }: Props) => {
  ...
  const [isShowMoaeraRetrievalModal, setIsShowMoaeraRetrievalModal] = useState(false)
    ...
    <div className={s.container}>
      <div className='px-6 py-3 flex flex-col'>
        ...
        <Textarea
          ...
          onClickRetrievalMethod={() => setIsShowMoaeraRetrievalModal(true)}
      ...
      <Drawer unmount={true} isOpen={isShowMoaeraRetrievalModal} onClose={() => setIsShowMoaeraRetrievalModal(false)} footer={null} mask={isMobile} panelClassname='mt-16 mx-2 sm:mr-2 mb-3 !p-0 !max-w-[640px] rounded-xl'>
        <MoaeraRetrievalModal
          indexMethod={currentDataset?.indexing_technique || ''}
          value={retrievalConfig}
          isShow={isShowMoaeraRetrievalModal}
          onHide={() => setIsShowMoaeraRetrievalModal(false)}
          onSave={(value) => {
            setRetrievalConfig(value)
            setIsShowMoaeraRetrievalModal(false)
          }}
        />
      </Drawer>
    </div>
